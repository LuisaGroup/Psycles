...
[[nodiscard]] bool
is_spatial_source(compiler::ValueOperation operation) noexcept {
...
[[nodiscard]] bool
is_light_path_source(compiler::ValueOperation operation) noexcept {
  using compiler::ValueOperation;
  switch (operation) {
...
private:
  enum class Visit : std::uint8_t { unseen, active, homogeneous, varying };

  const compiler::SurfaceProgram &_program;
  std::vector<Visit> _values;
  std::vector<Visit> _volumes;

  [[nodiscard]] bool value(compiler::ValueExpressionId id) noexcept {
    if (!id.valid()) {
      return true;
    }
    ...
    if (visit == Visit::varying || visit == Visit::active) {
      return false;
    }
    visit = Visit::active;
    const auto &instruction = _program.value_instructions()[id.value];
    auto homogeneous = !is_spatial_source(instruction.operation);
    const std::array dependencies{instruction.a, instruction.b, instruction.c,
                                  instruction.d, instruction.e, instruction.f,
                                  instruction.g, instruction.h, instruction.i,
                                  instruction.j};
    for (const auto dependency : dependencies) {
      const auto dependency_homogeneous = value(dependency);
      homogeneous = homogeneous && dependency_homogeneous;
    }
    visit = homogeneous ? Visit::homogeneous : Visit::varying;
    return homogeneous;
  }

  [[nodiscard]] bool volume(compiler::VolumeExpressionId id) noexcept {
    if (!id.valid()) {
      return true;
    }
    ...
    visit = Visit::active;
    const auto &instruction = _program.volume_instructions()[id.value];
    const std::array values{instruction.color,
                            instruction.density,
    ...
    const auto a_homogeneous = volume(instruction.a);
    const auto b_homogeneous = volume(instruction.b);
    homogeneous = homogeneous && a_homogeneous && b_homogeneous;
    visit = homogeneous ? Visit::homogeneous : Visit::varying;
    return homogeneous;
  }

public:
  explicit HomogeneousVolumeAnalysis(const compiler::SurfaceProgram &program)
      : _program{program},
        _values(program.value_instructions().size(), Visit::unseen),
        _volumes(program.volume_instructions().size(), Visit::unseen) {}
...
VolumeProgramCapabilities VolumeProgramCapabilityComponent::analyze(
    const compiler::SurfaceProgram &program) const noexcept {
  auto has_spatial_values = false;
  auto has_light_path = false;
  for (const auto &instruction : program.value_instructions()) {
    has_spatial_values =
        has_spatial_values || is_spatial_source(instruction.operation);
    has_light_path =
        has_light_path || is_light_path_source(instruction.operation);
  }
  HomogeneousVolumeAnalysis homogeneous{program};
  const auto is_homogeneous = homogeneous.analyze();
  return {.has_spatial_values = has_spatial_values,
          .homogeneous = is_homogeneous,
          .has_light_path = has_light_path};
}

void VolumeProgramCapabilityComponent::merge_surface_flags(
    std::vector<std::uint32_t> &flags, std::uint32_t surface_tag,
    const compiler::SurfaceProgram &program) const {
  if (flags.size() <= surface_tag) {
    flags.resize(static_cast<std::size_t>(surface_tag) + 1u, 0u);
  }
  const auto capabilities = analyze(program);
  auto &surface_flags = flags[surface_tag];
  surface_flags |=
      capabilities.homogeneous ? 0u : volume_surface_flag_heterogeneous;
  surface_flags |=
      capabilities.has_light_path ? volume_surface_flag_light_path : 0u;
}
