...
public:
  ...
  emit(const std::shared_ptr<LuisaSceneData> &scene,
    ...
    const auto object_ray =
        make_ray(object_origin, object_direction, world_ray->t_min(),
                 world_ray->t_max());
    auto intersection =
        _ribbon->intersect(object_ray, primitive.curve.control_points,
                           primitive.curve.geometry.curve_subdivision_level);
    ...
    const auto unit_object_direction = select(
        object_segment, object_segment / object_distance,
    ...
    const auto intercepts = scene->heap->buffer<float>(
        primitive.curve.geometry.bindless_base + 3u);
    const auto intercept_begin =
        intercepts.read(primitive.curve.segment.key_begin);
    const auto intercept_end =
        intercepts.read(primitive.curve.segment.key_end);
    const auto intercept =
        lerp(intercept_begin, intercept_end, intersection.u);
    const auto curve_length =
        scene->heap
            ->buffer<float>(primitive.curve.geometry.bindless_base + 5u)
            .read(primitive.curve.segment.curve_index);
    const auto curve_random =
        scene->heap
            ->buffer<float>(primitive.curve.geometry.bindless_base + 6u)
            .read(primitive.curve.segment.curve_index);
    ...
    const auto tangent_normal = normalize(
        incoming - dpdu * (dot(dpdu, incoming) / dot(dpdu, dpdu)));
...
std::shared_ptr<const CurveGeometryComponent>
