...
/* Exact CLOSURE_BSDF_HAIR_CHIANG_ID transition from Cycles 5.2.1
 * svm_node_closure_bsdf(). The cursor initially names the first word of
 * SVMNodePrincipledHairBsdfData and leaves it immediately after all 26 words.
 */
void node_principled_hair_chiang(
    const KernelGlobals &kernel_globals, Cursor &cursor, Stack &stack,
    luisa::compute::Expr<luisa::float3> closure_weight,
    luisa::compute::Expr<float> mix_weight, ShaderData &shader_data) noexcept;
