...
void SurfaceProgramBuilder::diagnose(
...
[[nodiscard]] ParameterId SurfaceProgramBuilder::add_parameter(
    const contract::ShaderNode &node,
...
[[nodiscard]] ValueExpressionId SurfaceProgramBuilder::append(
...
[[nodiscard]] ClosureExpressionId SurfaceProgramBuilder::append(
...
[[nodiscard]] VolumeExpressionId SurfaceProgramBuilder::append(
...
[[nodiscard]] std::optional<Id> SurfaceProgramBuilder::source_output(
    const contract::ShaderNode &node,
    std::string_view socket,
...
SurfaceProgramBuilder::lower_value_input(
    const contract::ShaderNode &node,
    std::string_view socket) {
...
SurfaceProgramBuilder::lower_closure_input(
    const contract::ShaderNode &node,
    std::string_view socket) {
    const auto *binding = find_input(node, socket);
...
SurfaceProgramBuilder::lower_volume_input(
    const contract::ShaderNode &node,
    std::string_view socket) {
    const auto *binding = find_input(node, socket);
    if (binding == nullptr || !binding->source) {
...
void SurfaceProgramBuilder::publish(
...
void SurfaceProgramBuilder::lower_node(
    const contract::ShaderNode &node) {
    if (lower_context_node(node) ||
        lower_value_node(node) ||
        lower_texture_node(node) ||
        lower_closure_node(node)) {
        return;
    }
    diagnose(
