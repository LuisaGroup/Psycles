...
public:
  ...
  [[nodiscard]] Float
  cycles_bsdf_data(Expr<std::uint32_t>) const noexcept override {
    return 0.73f;
  }
...
enum class ProbeContract {
  selected_closure_sample,
  conditional_sample,
};

[[nodiscard]] constexpr std::string_view
probe_contract_name(ProbeContract contract) noexcept {
  switch (contract) {
  case ProbeContract::selected_closure_sample:
    return "selected_closure_sample";
  case ProbeContract::conditional_sample:
    return "conditional_sample";
  }
  return "unknown";
}

[[nodiscard]] ProbeContract parse_probe_contract(std::string_view name) {
  if (name == "selected_closure_sample") {
    return ProbeContract::selected_closure_sample;
  }
  if (name == "conditional_sample") {
    return ProbeContract::conditional_sample;
  }
  throw std::invalid_argument{
      "contract must be selected_closure_sample or conditional_sample"};
}

[[nodiscard]] constexpr SurfaceClosureReachability
kind_reachability(SurfaceClosureKind kind,
  ...
  return {.kinds = kind_bit,
          .principled_lobes = kind == SurfaceClosureKind::principled
...
[[nodiscard]] auto make_probe_kernel(const ProbeSpec &spec,
  ...
  return Kernel1D{[spec, contract](BufferFloat4 input, BufferFloat4 output,
                                   UInt iterations) noexcept {
    set_block_size(256u);
    const auto seed = input.read(dispatch_x());
    const auto normal =
        normalize(make_float3(seed.x * 0.25f, seed.y * 0.25f, 1.0f));
    const auto incoming = normalize(
        make_float3(0.13f + seed.x * 0.1f, -0.21f + seed.y * 0.1f, 0.968f));
    const SurfaceClosurePoint point{normal, normal, incoming, 0xffffffffu,
                                    false,  false,  false};
    ...
    closure_record.normal =
        spec.kind == SurfaceClosureKind::thin_glass_transmission ? -normal
                                                                 : normal;
    ...
    Float2 random = clamp(seed.zw(), make_float2(0.001f), make_float2(0.999f));
    Float4 accumulated = make_float4(0.0f);
    UInt iteration = 0u;
    $while(iteration < iterations) {
      const auto sample = surface_closure_conditional_sample(
          services, point, normal, closure, incoming, normal, random, random.x,
          query, spec.reachability);
      if (contract == ProbeContract::conditional_sample) {
        // This contract isolates p(w | i). It intentionally consumes
        // every observable sampler result needed to prevent dead-code
        // elimination from turning the measurement into a direction-
        // only microbenchmark. The selected-closure contract below
        // additionally measures the independent f(w), p(w) replay.
        accumulated += make_float4(
            sample.direction.x + sample.direction.y,
            sample.roughness.x + sample.roughness.y,
            sample.eta + sample.singular_pdf + sample.singular_evaluation.x,
            cast<float>(sample.properties) + select(0.0f, 1.0f, sample.valid));
      } else {
        const auto contribution = surface_closure_evaluation_contribution(
            services, point, normal, closure, incoming, sample.direction, query,
            policy, false, spec.reachability);
        using namespace surface_closure_sample_property;
        const auto transparent_sample = (sample.properties & transparent) != 0u;
        const auto singular_sample = (sample.properties & singular) != 0u;
        const auto delta_sample = transparent_sample | singular_sample;
        const auto selected_evaluation =
            select(contribution.f,
                   select(sample.singular_evaluation, make_float3(1.0e6f),
                          transparent_sample),
                   delta_sample);
        const auto selected_pdf =
            select(contribution.weighted_pdf,
                   select(sample.singular_pdf, 1.0e6f, transparent_sample),
                   delta_sample);
        accumulated +=
            make_float4(select(0.0f, selected_evaluation.x, sample.valid),
                        sample.direction.x + sample.direction.y,
...
int main(int argc, char **argv) {
  if (argc < 3 || argc > 7) {
    std::cerr << "usage: " << argv[0]
              << " BACKEND PROBE [COUNT [ITERATIONS [REPEATS "
                 "[CONTRACT]]]]\n"
                 "contracts: selected_closure_sample "
                 "conditional_sample\nprobes:";
    for (const auto &probe : probe_specs) {
      std::cerr << ' ' << probe.name;
    }
  ...
  try {
    ...
    const auto iterations = argc > 4 ? parse_u32(argv[4], "iterations") : 32u;
    const auto repeats = argc > 5 ? parse_u32(argv[5], "repeats") : 5u;
    const auto contract = argc > 6 ? parse_probe_contract(argv[6])
                                   : ProbeContract::selected_closure_sample;
    if (count == 0u || iterations == 0u || repeats == 0u) {
      throw std::invalid_argument{
          "count, iterations, and repeats must be positive"};
    }
    ...
    Kernel1D initialize = [](BufferFloat4 values) noexcept {
      set_block_size(256u);
      const auto lane = cast<float>(dispatch_x() & 255u) * (1.0f / 256.0f);
      values.write(dispatch_x(),
                   make_float4(0.13f, -0.17f, 0.001f + 0.997f * lane,
                               0.999f - 0.997f * lane));
    };
    auto initialize_shader = device.compile(initialize);
    auto shader = device.compile(make_probe_kernel(probe, contract));
    ...
    for (auto repeat = std::uint32_t{0u}; repeat < repeats; ++repeat) {
      ...
      const auto ns_per_handler = milliseconds * 1.0e6 /
                                  static_cast<double>(count) /
                                  static_cast<double>(iterations);
      std::cout << "repeat=" << repeat << " milliseconds=" << milliseconds
                << " ns_per_handler=" << ns_per_handler << '\n';
    }
    std::array<luisa::float4, 1u> checksum{};
    stream << output.view(0u, 1u).copy_to(std::span{checksum}) << synchronize();
    std::cout << "checksum=" << checksum[0u].x << ',' << checksum[0u].y << ','
              << checksum[0u].z << ',' << checksum[0u].w << " count=" << count
              << " iterations=" << iterations << " warmups=" << warmup_count
              << " contract=" << probe_contract_name(contract)
              << " probe=" << probe.name << '\n';
    return EXIT_SUCCESS;
