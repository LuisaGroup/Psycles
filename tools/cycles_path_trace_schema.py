...
def scheduler_exact_component_mask(slot: TraceSlot) -> int:
    """Same-implementation comparisons have no topology/diagnostic exemption.

    Reuse the oracle field policies, but require identical primitive identity
    for a scheduler-only change and also compare its traversal diagnostics.
    Barycentrics remain continuous. The fourth lane is always a written bit.
    """
    if slot.name == "shadow_hit_id":
        policies = (COMPARE_EXACT,) * 3
    elif slot.name in {"shadow_hit_coord", "shadow_transmittance"}:
        policies = (COMPARE_FLOAT32,) * 3
    else:
        policies = comparison_policies(slot)
    mask = 1 << 3
    for component, policy in enumerate(policies):
        continuous = policy == COMPARE_FLOAT32 or (
            policy == COMPARE_TOPOLOGY and slot.components[component] in {"u", "v"}
        )
        if not continuous:
            mask |= 1 << component
    return mask
...
def cpp_scheduler_comparison_header() -> str:
    """Host-readback metadata only; does not change the trace ABI or shaders."""
    return "\n".join([
        "#pragma once",
        "",
        "// Generated by tools/cycles_path_trace_schema.py. Do not edit.",
        ...
        *[f"        0x{scheduler_exact_component_mask(slot):x}u, // {slot.index}: {slot.name}"
          for slot in SLOTS],
        "};",
        "}// namespace psycles::luisa_backend::path_trace_schema",
        "",
