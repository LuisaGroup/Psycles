...
This checkpoint re-renders Classroom, Lone Monk, Monster Under the Bed, and
the official Blender benchmark from fresh Blender 5.2.1 exports after the
Cycles Fast GI and emission-endpoint implementation at `debbadb`. The renderer
and documentation head is `b3c65d2`. Every Cycles reference comes from commit
...
Representative stable GPU samples reach 100% busy at 241 W for Classroom and
267 W for Benchmark. Lower transient samples occur around queue transitions
and scene setup. Kernel profiling remains necessary to explain why full-scene
gaps range from 7.5% to 54.3% despite saturation.
...
[Monster](monster/all-pass-report.json), and
