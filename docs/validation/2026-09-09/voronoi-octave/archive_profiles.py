"""Freeze native Voronoi octave A/B/B/A evidence, without a CPU shader oracle."""
import argparse
import csv
import json
...
def main():
    ...
    for name in ('before', 'shared_first', 'shared_repeat', 'restored', 'output'):
        parser.add_argument(name, type=Path)
    parser.add_argument('--final', type=Path,
                        help='separate follow-up with the homogeneous-position correction too')
    args = parser.parse_args()
    paths = {k: v for k, v in vars(args).items() if k != 'output' and v is not None}
    ...
    for name, path in paths.items():
        ...
        if name != 'before':
            del actual
    for a, b in (('before', 'restored'), ('shared_first', 'shared_repeat')):
    ...
    result = {
        'schema': 'psycles.voronoi-octave-profiles.v1',
        ...
        'intervention': 'runtime octave selection restores the original single shared F1 fallback for ordinary F1 and zero-smoothness Smooth F1',
        'final_followup_scope': 'optional final run additionally restores native 3D homogeneous position W; not included in the isolated shared-F1 medians',
        'unchanged': 'inlining policy, fast math, register limits, launch geometry, RNG, frame, SVM word images and static allocation analysis',
        ...
        'medians': {'before': median(('before', 'restored')),
                    'shared': median(('shared_first', 'shared_repeat'))},
    }
