"""Freeze full Barbershop surface emission eligibility A/B/B/A evidence."""
import argparse
import csv
import json
...
def main():
    ...
    for name, path in paths.items():
        ...
        if name != 'before':
            del actual
    for a, b in (('before', 'restored'), ('guarded_first', 'guarded_repeat')):
    ...
    result = {
        'schema': 'psycles.surface-emission-profiles.v1',
        ...
        'intervention': 'native SD_EMISSION and BSSRDF-exit eligibility guard forward MIS and atomic surface emission',
        'unchanged': 'inlining policy, fast math, register limits, launch geometry, RNG, SVM word images, static allocation analysis and direct/indirect split-film representation',
        ...
        'medians': {'before': median(('before', 'restored')),
                    'guarded': median(('guarded_first', 'guarded_repeat'))},
    }
