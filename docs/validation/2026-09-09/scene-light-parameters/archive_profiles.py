"""Freeze full Barbershop scene-owned light parameter A/B/B/A evidence."""
import argparse
import csv
import json
...
def main():
    ...
    for name in ('before', 'prepared_first', 'prepared_repeat', 'restored', 'output'):
    ...
    for name, path in paths.items():
        ...
        if name != 'before':
            del actual
    for a, b in (('before', 'restored'), ('prepared_first', 'prepared_repeat')):
    ...
    result = {
        'schema': 'psycles.scene-light-parameter-profiles.v1',
        ...
        'intervention': 'native scene-owned area/spot parameters replace device trigonometry and restore original parameter-consuming predicates/arithmetic',
        ...
        'medians': {'before': median(('before', 'restored')),
                    'prepared': median(('prepared_first', 'prepared_repeat'))},
    }
