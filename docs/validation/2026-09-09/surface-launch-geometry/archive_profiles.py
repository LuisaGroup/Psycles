"""Freeze a surface block-size A/B/B/A, without shader or timing oracles."""
import argparse
import csv
import json
...
def main():
    ...
    for name in ('before', 'block_1024', 'repeat_1024', 'restored', 'output'):
    ...
    for name, path in paths.items():
        ...
        assert blocks == {((512 if name in ('before', 'restored') else 1024), 1, 1)}
        ...
        if name != 'before':
            del actual
    for a, b in (('before', 'restored'), ('block_1024', 'repeat_1024')):
        assert rows[a]['surface_text']['sha256'] == rows[b]['surface_text']['sha256']
        assert rows[a]['command']['executor_sha256'] == rows[b]['command']['executor_sha256']
    ...
    result = {
        'schema': 'psycles.surface-launch-geometry.v1',
        'implementation': {'parent': 'e79644af', 'child': 'da8fff856',
                           'scope': 'only temporary surface continuation block size changed; production restored'},
        ...
        'intervention': 'surface execution_block_size 512 to 1024 in the generic continuation configuration',
        'unchanged': 'inlining policy, fast math, register limits, RNG identity, frame, sorting Handler, SVM images and static allocation analysis',
        ...
        'evidence': [source(args.before.parent / name) for name in (
            'formal-analysis.md', 'run_profile.py', 'before_path_kernel_executor.cpp',
            'build-1024.log', 'build-restored.log')],
        'runs': rows,
        'medians': {'block_512': median(('before', 'restored')),
                    'block_1024': median(('block_1024', 'repeat_1024'))},
        'decision': 'reject 1024 for production; matching original register counts did not improve performance',
    }
