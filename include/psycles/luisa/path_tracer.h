...
[[nodiscard]] constexpr bool luisa_path_scheduler_uses_cycles_stage_partition(
    LuisaPathScheduler scheduler) noexcept {
  switch (scheduler) {
  case LuisaPathScheduler::wavefront:
  case LuisaPathScheduler::wavefront_graph:
...
struct LuisaSurfaceClosureCountHistogram {
  std::array<std::uint64_t, luisa_surface_closure_count_histogram_bin_count>
      counts{};
...
struct LuisaSurfaceValueHandlerTransitionExecutionCount {
  ...
  auto operator<=>(const LuisaSurfaceValueHandlerTransitionExecutionCount &)
...
struct LuisaSurfaceValueOperandExecutionCounts {
  ...
  auto operator<=>(
      const LuisaSurfaceValueOperandExecutionCounts &) const noexcept = default;
};
...
struct LuisaSurfaceValueUniqueParameterCounts {
  ...
  auto operator<=>(
      const LuisaSurfaceValueUniqueParameterCounts &) const noexcept = default;
};
...
struct LuisaSurfaceValueRegionShape {
  ...
  auto
  operator<=>(const LuisaSurfaceValueRegionShape &) const noexcept = default;
};
...
inline constexpr std::size_t luisa_surface_parameter_reuse_bin_count = 8u;
...
struct LuisaSurfaceParameterReuseBin {
  ...
  auto
  operator<=>(const LuisaSurfaceParameterReuseBin &) const noexcept = default;
};
...
struct LuisaSurfaceProgramExecutionHistogram {
  ...
  // Retained ABI fields for the removed split-stream region experiment. A
  // unified structured SVM has no canonical maximal linear-region partition;
  // these fields are therefore empty/zero rather than projecting a false
  // correspondence across guards.
  ...
  std::array<std::array<LuisaSurfaceParameterReuseBin,
                        luisa_surface_parameter_reuse_bin_count>,
             3u>
      parameter_reuse_bins{};
  // Dense unified control-record order: ClosureLeaf, MixClosure,
  // AddClosureWeight, conditional Jump. Counts are the exact static program
  // projection weighted by measured topology populations. Guarded records are
  // potential visits, not dynamically observed branch outcomes; a zero-weight
  // leaf is likewise not mislabeled as a physical-closure allocation.
  ...
  // True means both the measured invocation counters and the host-side static
  // unified-PC projection were exact. It does not claim that guarded PCs were
  // dynamically visited; per-PC device counters are a distinct diagnostic.
  bool exact{};
...
struct LuisaPathTracerOptions {
  ...
  std::optional<LuisaSurfaceClosureCountHistogramRequest>
