...
constexpr std::array<std::uint32_t, 21u> default_words{
...
constexpr std::array<std::uint32_t, 23u> authored_words{
...
[[nodiscard]] consteval std::array<std::uint32_t, 24u>
make_mixed_default_words(std::uint32_t mix_bits) noexcept {
  std::array<std::uint32_t, 24u> result{};
  result[0u] = default_words[0u];
  result[1u] = 4u;
  result[2u] = 22u;
  result[3u] = 23u;
  ...
  result[5u] = mix_bits;
  result[6u] = 3u;
  for (auto index = 4u; index < default_words.size(); ++index) {
    result[index + 3u] = default_words[index];
  }
  result[16u] = 3u;
  return result;
}
...
constexpr auto quarter_mix_words =
    make_mixed_default_words(std::bit_cast<std::uint32_t>(0.25f));
constexpr auto zero_mix_words = make_mixed_default_words(0u);
  ...
  return Kernel1D<Buffer<std::uint32_t>, Buffer<luisa::float4>,
      ...
      [used](BufferUInt words, BufferFloat4 output, BufferUInt meta,
        ...
        $if(closures.count() != 0u) {
          const auto closure = closures.ray_portal(0u);
          output.write(
              output_base + 0u,
          ...
          output.write(output_base + 2u, make_float4(closure.param.P, 0.0f));
          output.write(output_base + 3u, make_float4(closure.param.D, 0.0f));
...
[[nodiscard]] bool run(std::string_view backend, char **argv) {
  ...
  auto signed_weight = default_words;
  signed_weight[8u] = std::bit_cast<std::uint32_t>(-0.3f);
  signed_weight[9u] = std::bit_cast<std::uint32_t>(0.6f);
  signed_weight[10u] = std::bit_cast<std::uint32_t>(0.3f);
  auto below_cutoff = default_words;
  below_cutoff[8u] = std::bit_cast<std::uint32_t>(1.0e-6f);
  below_cutoff[9u] = std::bit_cast<std::uint32_t>(1.0e-6f);
  below_cutoff[10u] = std::bit_cast<std::uint32_t>(1.0e-6f);
  auto nan_weight = default_words;
  nan_weight[8u] = 0x7fc00000u;

  auto default_buffer =
      device.create_buffer<std::uint32_t>(default_words.size());
  auto authored_buffer =
      device.create_buffer<std::uint32_t>(authored_words.size());
  auto quarter_buffer =
      device.create_buffer<std::uint32_t>(quarter_mix_words.size());
  auto signed_buffer =
      device.create_buffer<std::uint32_t>(signed_weight.size());
  auto low_buffer = device.create_buffer<std::uint32_t>(below_cutoff.size());
  auto nan_buffer = device.create_buffer<std::uint32_t>(nan_weight.size());
  ...
  stream << default_buffer.copy_from(default_words.data())
         << authored_buffer.copy_from(authored_words.data())
         << quarter_buffer.copy_from(quarter_mix_words.data())
         << signed_buffer.copy_from(signed_weight.data())
         << low_buffer.copy_from(below_cutoff.data())
         << nan_buffer.copy_from(nan_weight.data())
         << zero_mix_buffer.copy_from(zero_mix_words.data())
         << full_shader(default_buffer, output, meta, 0u).dispatch(1u)
         << full_shader(authored_buffer, output, meta, 1u).dispatch(1u)
         << full_shader(quarter_buffer, output, meta, 2u).dispatch(1u)
         << full_shader(signed_buffer, output, meta, 3u).dispatch(1u)
         << full_shader(low_buffer, output, meta, 4u).dispatch(1u)
         << full_shader(nan_buffer, output, meta, 5u).dispatch(1u)
         << empty_shader(default_buffer, output, meta, 6u).dispatch(1u)
  ...
  constexpr auto portal_type =
      static_cast<std::uint32_t>(CLOSURE_BSDF_RAY_PORTAL_ID);
  constexpr auto expected_flags =
      device_svm::shader_data_bsdf | device_svm::shader_data_ray_portal;
  constexpr auto ended =
      static_cast<std::uint32_t>(device_svm::EvaluationStatus::ended);
  const auto default_weight = luisa::float3{0.26f, 0.71f, 0.43f};
  const auto shader_position = luisa::float3{4.0f, -2.0f, 0.5f};
  const auto fallback_direction = luisa::float3{0.0f, -0.6f, -0.8f};

  auto valid = state(0u, 0u) == portal_type && state(0u, 1u) == 1u &&
               state(0u, 2u) == 7u && state(0u, 3u) == expected_flags &&
               state(0u, 4u) == ended && state(0u, 5u) == 19u &&
               near(value(0u, 0u).xyz(), default_weight) &&
               near(value(0u, 0u).w, 0.46666666865348816f) &&
               near(value(0u, 1u).xyz(), luisa::float3{0.0f, 0.0f, 1.0f}) &&
               near(value(0u, 2u).xyz(), shader_position) &&
               near(value(0u, 3u).xyz(), fallback_direction) &&
               near(value(0u, 4u).xyz(), default_weight);

  valid &= state(1u, 0u) == portal_type && state(1u, 1u) == 1u &&
           state(1u, 3u) == expected_flags && state(1u, 5u) == 21u &&
           near(value(1u, 0u).xyz(), luisa::float3{0.83f, 0.17f, 0.52f}) &&
           near(value(1u, 0u).w, 0.5066666603088379f) &&
           near(value(1u, 2u).xyz(), luisa::float3{1.25f, -0.75f, 2.5f}) &&
           near(value(1u, 3u).xyz(),
                luisa::float3{3.0f / 13.0f, -4.0f / 13.0f, 12.0f / 13.0f}) &&
           near(value(1u, 4u).xyz(), luisa::float3{0.83f, 0.17f, 0.52f});

  valid &= state(2u, 0u) == portal_type && state(2u, 5u) == 22u &&
           near(value(2u, 0u).xyz(), default_weight * 0.25f) &&
           near(value(2u, 0u).w, 0.46666666865348816f * 0.25f) &&
           near(value(2u, 4u).xyz(), default_weight * 0.25f);

  const auto signed_value = luisa::float3{-0.3f, 0.6f, 0.3f};
  valid &=
      state(3u, 0u) == portal_type && near(value(3u, 0u).xyz(), signed_value) &&
      near(value(3u, 0u).w, 0.2f) && near(value(3u, 4u).xyz(), signed_value);

  for (auto scenario : {4u, 5u, 7u}) {
    valid &=
        ...
        near(value(scenario, 4u).xyz(), luisa::float3{0.0f});
  }
  valid &= state(4u, 5u) == 19u && state(5u, 5u) == 19u && state(7u, 5u) == 22u;

  /* Cycles accumulates extinction before closure_alloc, so capacity failure
   * is not an identity transition on ShaderData. */
  valid &= state(6u, 0u) == static_cast<std::uint32_t>(CLOSURE_NONE_ID) &&
           state(6u, 1u) == 0u && state(6u, 2u) == 0u && state(6u, 3u) == 0u &&
           state(6u, 4u) == ended && state(6u, 5u) == 19u &&
           near(value(6u, 4u).xyz(), default_weight);

  if (!valid) {
    std::cerr << "Cycles standalone Ray Portal transition mismatch on "
              << backend << '\n';
    for (auto scenario = 0u; scenario < scenario_count; ++scenario) {
      const auto common = value(scenario, 0u);
      const auto extinction = value(scenario, 4u);
      std::cerr << "scenario " << scenario << ": type=" << state(scenario, 0u)
                ...
                << common.w << "), extinction=(" << extinction.x << ", "
                << extinction.y << ", " << extinction.z << ")\n";
    }
  }
  return valid;
}
