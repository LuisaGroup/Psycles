...
#include "cycles_svm_shadow_surface_fixture.h"
...
constexpr auto case_count =
    static_cast<unsigned>(psycles::test_support::shadow_surface_case_count);
constexpr auto features =
    svm::kernel_feature_hair | svm::kernel_feature_node_principled_hair |
...
void evaluate(const Globals &kg, const Words &words,
              ...
              bool diagnose) {
  constexpr auto mask = svm::kernel_feature_node_mask_surface_shadow;
  if (!diagnose) {
    if constexpr (requires {
                    eval_nodes_assume_valid(kg, words, SHADER_TYPE_SURFACE,
                                            features, mask, used, transforms,
                                            sd, state, 1u);
                  }) {
      eval_nodes_assume_valid(kg, words, SHADER_TYPE_SURFACE, features, mask,
                              used, transforms, sd, state, 1u);
      return;
    }
  }
  eval_nodes(kg, words, SHADER_TYPE_SURFACE, features, mask, used, transforms,
             sd, state, result);
}
...
auto make_kernel(bool has_storage, bool diagnose) {
  return Kernel1D<Buffer<unsigned>, Buffer<float>, Buffer<float>,
      ...
      [=](BufferUInt words, BufferFloat tables, BufferFloat cosines,
        ...
        const svm::PathState state{svm::path_ray_visibility_shadow, 0u};
...
bool run(int argc, char **argv) {
  ...
  const auto image = psycles::test_support::make_shadow_surface_image();
  const auto table =
  ...
  for (auto i = 0u; i < case_count; ++i) {
    cosine_values[i] = psycles::test_support::shadow_surface_cosine(i);
  }
  ...
  for (const auto has_storage : {false, true}) {
    for (const auto diagnose : {true, false}) {
      ...
      stream
          ...
          << synchronize();
      std::ifstream oracle{PSYCLES_SHADOW_SURFACE_ORACLE};
      if (!oracle) {
        std::cerr << "Missing Cycles shadow-surface oracle\n";
        return false;
      }
      bool variant_passed = true;
      for (auto i = 0u; i < case_count; ++i) {
        unsigned scenario{}, flags{}, offset{}, count{}, left{}, lcg{};
  ...
  if (passed) {
    std::cout << "Cycles closure-free shadow-surface tests passed\n";
  }
  return passed;
}
