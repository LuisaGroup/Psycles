...
constexpr std::array sample_cases{
    // Exact first-bounce diffuse sample recorded by the Cycles path oracle.
...
int main(int argc, char **argv) {
    ...
    Kernel1D evaluate =
        [](BufferFloat4 input,
           ...
           BufferFloat4 bitangents) noexcept {
            const auto index = dispatch_x();
            const auto normal = input.read(index).xyz();
            const auto basis =
                sample_mapping::make_orthonormals(normal);
            const auto sample =
                sample_mapping::sample_cosine_hemisphere(
                    normal,
                    random.read(index));
            samples.write(
                index,
            ...
            tangents.write(
                index,
                make_float4(basis.tangent, 0.0f));
            bitangents.write(
                index,
                make_float4(basis.bitangent, 0.0f));
        };
    ...
    for (std::size_t i = 0u; i < sample_cases.size(); ++i) {
        ...
        if (!approximately_equal(
                actual_direction,
                sample_cases[i].expected_direction) ||
            !approximately_equal(
                samples[i].w,
                sample_cases[i].expected_pdf)) {
            std::cerr
                << "Cycles cosine-hemisphere mapping failed on "
                << backend << " for case " << i
                << ": direction {" << actual_direction.x
                << ", " << actual_direction.y
                << ", " << actual_direction.z
                << "}, pdf " << samples[i].w
                << '\n';
        ...
        if (!approximately_equal(
                ...
                0.0f) ||
            !approximately_equal(
                dot(bitangent, normal),
                0.0f) ||
            !approximately_equal(
                dot(tangent, bitangent),
                0.0f) ||
            !approximately_equal(
                dot(tangent, tangent),
                1.0f) ||
            !approximately_equal(
                ...
                1.0f)) {
            std::cerr
                << "orthonormal-basis invariant failed on "
                << backend << " for case " << i << '\n';
            return EXIT_FAILURE;
        }
    }

    // The equal-component branch is easy to lose in a generic basis helper.
    // Lock its Cycles-defined orientation, not merely its orthogonality.
    constexpr auto expected_equal_tangent =
        luisa::float3{
            0.0f,
            inverse_sqrt_two,
            -inverse_sqrt_two};
    constexpr auto expected_equal_bitangent =
        luisa::float3{
            -sqrt_two_thirds,
            inverse_sqrt_six,
            inverse_sqrt_six};
    if (!approximately_equal(
            tangents[3u].xyz(),
            expected_equal_tangent) ||
        !approximately_equal(
            bitangents[3u].xyz(),
            expected_equal_bitangent)) {
        std::cerr
