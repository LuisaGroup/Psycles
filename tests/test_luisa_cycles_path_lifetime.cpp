...
struct Case {
  ...
  unsigned transparent_maximum;
};
constexpr std::array cases{
    Case{"boundary-1", 2320u, 0u}, Case{"boundary-2", 4624u, 0u},
    Case{"boundary-2-budget-control", 4624u, 2u}};

struct Bundle {
  std::filesystem::path path = std::filesystem::temp_directory_path() /
      ("psycles-path-lifetime-" + std::to_string(
          std::chrono::steady_clock::now().time_since_epoch().count()));
  explicit Bundle(const Case &input) {
    require(std::filesystem::create_directory(path), "cannot create test bundle");
    const auto prefix = std::filesystem::path{PSYCLES_PATH_LIFETIME_FIXTURES} / input.name;
...
bool render(luisa::compute::Device &device, const Case &input,
  ...
  for (unsigned row = 0; row < 256u; ++row) {
    ...
    for (unsigned lane = 0; lane < 3u; ++lane) {
      ...
      mismatches += !std::isfinite(actual) || std::abs(actual - expected[lane]) > tolerance;
    }
  }
  std::string extra;
...
int main(int argc, char **argv) {
  try {
    ...
    for (const auto &input : cases) {
      ...
      require(imported.ok(), "original path lifetime bundle failed to import");
      require(imported.integrator.max_bounces == 0u &&
                  imported.integrator.transparent_max_bounces == input.transparent_maximum,
              "original bounce controls were changed");
      const auto compiled = compiler.compile_scene(*imported.scene);
      for (const auto &diagnostic : compiled.diagnostics) {
        std::cerr << diagnostic.message << '\n';
      }
      require(compiled.ok(), "original path lifetime scene failed to compile");
